...
export function SmartAssistant({bridge, onClose}: {bridge:TerminalBridge; onClose:()=>void}) {
  ...
  const submit=async()=>{const id=++generation.current;setBusy(true);setError('');setText('');try{const result=await bridge.suggest({prompt});if(id===generation.current)setText(result.text)}catch{if(id===generation.current)setError('暂时无法生成建议，请检查 DSH 的模型设置后重试。')}finally{if(id===generation.current)setBusy(false)}};
