...
function WorkspaceSession({ bridge, sessionId }: TerminalWorkspaceProps) {
  ...
  const refresh = useCallback(async (manual = false) => {
    ...
    try {
      ...
      if (aliveRef.current && sequence === listSequence.current) setListError(`终端列表暂不可用：${errorText(error)}`);
    } finally {
  ...
  return (
    <div className="dsh-terminal-workspace" data-session-id={sessionId} onKeyDownCapture={onKeyDown}>
      <style>{workspaceCss}</style>
      <style>{xtermCss}</style>
      <header className="dt-workspace-bar">
        <div className="dt-brand"><span className="dt-brand-mark">&gt;_</span><strong>DSH Terminal</strong><span className="dt-brand-subtitle">原生 CLI 工作台</span></div>
      ...
      <div className="dt-workspace-context"><span className="dt-context-label">工作目录</span><span className="dt-cwd" title={cwd}>{cwd || (loading ? '读取中…' : '暂不可用')}</span><span className="dt-context-hint">CLI 使用独立配置，首次使用可能需要登录</span></div>
        ...
        {slots.map((terminal, index) => {
          ...
          return <div className="dt-cell" key={terminal?.id ?? `empty-${index}`} style={style} aria-hidden={!visible}>
            {terminal ? <PaneBoundary><TerminalPane
              terminal={terminal} bridge={bridge} viewerId={viewerId} number={index + 1}
            ...
            /></PaneBoundary> : <div className={`dt-empty-pane${selectedSlot === index ? ' is-selected' : ''}`}
              ...
              <strong>{opening[index] === 'pending' ? '正在启动…' : opening[index] === 'uncertain' ? '等待确认启动结果' : '启动一个终端'}</strong>
              <span className="dt-empty-description">原生界面，独立进程</span>
              <div className="dt-launchers">{launchers.map(launcher => <button key={launcher.id}
                ...
                onClick={() => { void open(index, launcher.id); }}>{launcher.label}{!launcher.available && <span>未安装</span>}</button>)}</div>
