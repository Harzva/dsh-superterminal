...
export const requests = {
  ...
  runResult: z.object({ terminalId: boundedId, messageId: boundedId.regex(/^assistant-\d+-\d+$/), offset: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER), limit: z.number().int().min(1).max(16000).optional() }).strict(),
  ...
  handoffList: z.object({ includeDiscussions: z.boolean().optional() }).strict(),
